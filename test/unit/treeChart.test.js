...
describe('TreeChart', () => {
  ...
  it('opens person details without also selecting a new chart root', () => {
    const chart = new TreeChart()
    ...
    const opened = []
    const selected = []
    chart.node.addEventListener('nav', event => opened.push(event.detail))
    chart.node.addEventListener('pedigree:person-selected', event =>
      selected.push(event.detail)
    )
    const button = nodeWithKey(chart, 'p').querySelector('.open-person-btn')
    expect(button.getAttribute('aria-label')).toBe('Person details')
    button.dispatchEvent(new MouseEvent('click', {bubbles: true}))
    expect(opened).toEqual([{path: 'person/I_R'}])
