...
export function appendPersonCard(
  ...
  {
    profile,
    surname = d => profile(d)?.name_surname,
    imageUrl = () => '',
    boxWidth = 190,
    boxHeight = 90,
    imgPadding = 10,
  ...
  const lines = [
    {
      ...
      text: (p, d) =>
        surnameFirst
          ? `${surname(d) || '…'},`
          : personGivenNameFromProfile(p) || '…',
      weight: 500,
    },
    {
      show: hasName,
      text: (p, d) =>
        surnameFirst ? personGivenNameFromProfile(p) || '…' : surname(d) || '…',
      weight: 500,
    },
    {
      show: p => p?.birth?.date,
      text: p => `*${formatDateString(p.birth.date, locale)}`,
      weight: 350,
    },
    {
      show: p => p?.death?.date,
      text: p => `†${formatDateString(p.death.date, locale)}`,
      weight: 350,
    },
  ]
  lines.forEach((line, i) => {
    nodes
      ...
      .text(d =>
        clipString(line.text(profile(d), d), boxWidth - textPadding(d))
      )
  })
...
export function setPersonCardInteraction(
  nodes,
  {
    profile,
    handle,
    grampsId = d => profile(d)?.gramps_id,
    openProfileLabel = '',
    onSelect,
    boxWidth = 190,
    boxHeight = 90,
  ...
  nodes
    ...
    .on('click', function (event, d) {
      ...
      if (onSelect) {
        onSelect.call(this, event, d)
      } else if (!canEdit && grampsId(d)) {
        fireEvent(this, 'pedigree:person-selected', {grampsId: grampsId(d)})
      }
    })
    .on('mouseenter', function (event, d) {
      const personId = grampsId(d)
      if (canEdit || !personId || !isHoverDevice()) {
        return
      }
...
export function clearPersonCardInteraction(nodes) {
  nodes
    ...
    .selectAll('.add-person-btn, .open-person-btn')
    .remove()
}
