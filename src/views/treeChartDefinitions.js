...
function relationshipGraphPeople({people, families}) {
  ...
  return people.map(person => ({
    ...
    extended: {
      ...
      families: (person.family_handles ?? [])
        .map(handle => familiesByHandle.get(handle))
        .filter(Boolean),
      primary_parent_family:
        familiesByHandle.get(person.primary_parent_family_handle) ?? {},
    },
  }))
}
...
export const chartDefinitions = {
  ancestor: {
    settings: [
      ancestorsSetting('treeChartAnc', DEFAULT_TREE_DEPTH),
  ...
  descendant: {
    settings: [
      descendantsSetting('descendantChartDesc', DEFAULT_TREE_DEPTH),
  ...
  hourglass: {
    settings: [
      ancestorsSetting('hourglassChartAnc', DEFAULT_TREE_DEPTH),
      descendantsSetting('hourglassChartDesc', DEFAULT_TREE_DEPTH),
  ...
  relationship: {
    ...
    dataUrl: (grampsId, {separation}, lang) =>
      `/api/views/relationship-graph/${encodeURIComponent(
        grampsId
      )}?degree=${separation}&locale=${lang || 'en'}`,
    getDataItems: relationshipGraphPeople,
  ...
  fan: {
    settings: [
      ancestorsSetting('fanChartAnc', DEFAULT_TREE_DEPTH),
  ...
  connection: {
    ...
    editable: false,
    zoomable: false,
    render: ({grampsId, targetGrampsId, appState}) => html`
      <grampsjs-view-connection-graph
        active
        grampsId=${grampsId}
        targetGrampsId=${targetGrampsId}
        .appState=${appState}
...
export function chartDataUrl(definition, grampsId, values, lang) {
  if (definition.dataUrl) {
    return definition.dataUrl(grampsId, values, lang)
  }
