...
export class GrampsjsViewTree extends GrampsjsStaleDataMixin(GrampsjsView) {
  ...
  get chart() {
    return normalizeTreeView(this.view)
  }
  ...
  willUpdate(changed) {
    ...
    if (changed.has('view')) {
      this._currentTabId = getTreeViewTabIndex(this.view)
    }
  ...
  _fetchIfNeeded() {
    if (
      !this.grampsId ||
      (!this.definition.request && !this.definition.dataUrl)
    ) {
      return
    }
  ...
  async _fetchData(url) {
    ...
    if ('data' in data) {
      this.error = false
      this._data = this.definition.getDataItems?.(data.data) ?? data.data
  ...
  _handleTabChange(e) {
    // A delayed event from a hidden chart must not navigate away from the
    // page that has just been opened from a person card.
    if (!this.active || this.appState?.path?.page !== 'tree') return
    const view = normalizeTreeView(
      e.detail?.value ?? getTreeViewForTab(e.target?.activeTabIndex)
    )
    fireEvent(this, 'edit-mode:off', {})
    fireEvent(this, 'nav', {
  ...
  async _handleHomePerson(e) {
    ...
    if (grampsId) {
      const result = this.appState.updateUserSettings
        ? await this.appState.updateUserSettings({homePerson: grampsId})
        : await this.appState.updateSettings({homePerson: grampsId}, true)
